...
const EditGameForm = ({ game, onClose, onSuccess }) => {
    const [formData, setFormData] = useState({
        ...
        genres: game.genres || [],
        coverImage: game.coverImage || game.thumbnail || '',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const gameData = {
                ...
                coverImage: formData.coverImage.trim() || game.coverImage,
                screenshots: formData.screenshots.filter(s => s.trim()),
                videoUrl: formData.videoUrl.trim(),
                lastModified: new Date().toISOString()
            };

            await dbOperations.update('games', game.id, gameData);
            ...
            setError('Oyun güncellenirken bir hata oluştu');
        } finally {
            setLoading(false);
        }
    };
